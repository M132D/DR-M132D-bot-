...
#=========={ادارة المواقع}==========#
if($text == "ادارة المواقع"){

$key = ['inline_keyboard'=>[]];

foreach(($addblusdel ?? []) as $code => $site){

// نتأكد إنه موقع (فيه add)
if(isset($site['add'])){
...
[
'text'=>"✏️ $name",
'callback_data'=>"edit_site_name-$code"
]
];
...
bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"✏️ اختر الموقع لتعديل اسمه",
'reply_markup'=>json_encode($key)
]);

}
if($exdata[0] == "edit_site_name"){

$code = $exdata[1];

file_put_contents("data/id/$id/step.txt","editname_$code");

bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"✏️ ارسل الاسم الجديد للموقع"
]);

}
if($text && strpos($step,"editname_") !== false){

$code = str_replace("editname_","",$step);

// نحفظ الاسم الجديد
$addblusdel[$code]['name'] = $text;

// حفظ الملف
file_put_contents("data/addblusdel.json", json_encode($addblusdel, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));

unlink("data/id/$id/step.txt");

bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"✅ تم تعديل اسم الموقع"
]);
...
if($text && $text != '/start' && $step == 'dollarh1h'){
    if($EMILS['emils'][$text]['emil'] == null){
        ...
        file_put_contents("data/id/$id/step.txt","dollarh1h2|$text");
    }
}
...
file_put_contents("data/id/$id/step.txt","dollar112|$text");
}
}
...
if($price == null){
$api = json_decode(file_get_contents("https://answs1234.darksidehost.com/jj/tele/GetBalance.php?key=6971786233a35a9b68643d2cb9619997d1193a1f05f7f8f2&from_id=1879396098"),1);
$price = $api['coin'];
}
if($price == null){
...
bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"⚙️ إدارة الاشتراك الاجباري",
'reply_markup'=>json_encode([
'inline_keyboard'=>[
[['text'=>"➕ إضافة قناة",'callback_data'=>"add_channel"]],
[['text'=>"➖ حذف قناة",'callback_data'=>"del_channel"]],
[['text'=>"📋 عرض القنوات",'callback_data'=>"list_channel"]]
]
])
]);
...
file_put_contents("data/add_channel.txt","on");

bot('editMessageText',[
'chat_id'=>$chat_id,
'message_id'=>$message_id,
'text'=>"📥 ارسل بهذا الشكل:

-1001234567890 channelusername"
]);
...
if(file_exists("data/add_channel.txt") && file_get_contents("data/add_channel.txt") == "on"){
...
file_put_contents("data/channels.json",json_encode($dataChannels,JSON_PRETTY_PRINT));

unlink("data/add_channel.txt");
...
if(isset($dataChannels['channels'])){

foreach($dataChannels['channels'] as $ch){
...
file_put_contents("data/del_channel.txt","on");
...
if(file_exists("data/del_channel.txt") && file_get_contents("data/del_channel.txt") == "on"){

$dataChannels = json_decode(@file_get_contents("data/channels.json"),true);

foreach($dataChannels['channels'] as $key=>$ch){

if($ch['id'] == $text){

unset($dataChannels['channels'][$key]);

}

}

$dataChannels['channels']=array_values($dataChannels['channels']);

file_put_contents("data/channels.json",json_encode($dataChannels,JSON_PRETTY_PRINT));

unlink("data/del_channel.txt");
...
$api = file_get_contents("http://vps.bot-sms.shop/stubs/handler_api.php?api_key=$api_key&action=getBalance");
$api = explode(":", $api);
$price = "$api[1] $";
$key[inline_keyboard][]=[['text'=>"</> bot-sms.shop",'callback_data'=>"Balapi-sms3t"],['text'=>"$price",'url'=>"bot-sms.shop/money"]];
}
if($addblusdel['onlinesim']['add'] == "ok"){
